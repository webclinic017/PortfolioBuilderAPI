...
class Portfolio(BaseModel):
    ...
    def normalise_index(self, timeseries: pd.DataFrame) -> pd.DataFrame:
        """
        Normalise given index to start with 1

        Args:
            timeseries (pd.DataFrame): Series to normalise

        Returns:
            pd.DataFrame: Normalised series
        """
    ...
    def backtest_portfolio(
        self, initial_amounts: List[float], timeseries: pd.DataFrame
    ) -> pd.DataFrame:
        """
        Backtest portfolio with no trading strategy (buy & hold)

        Args:
            initial_amounts (List[float]): Initial investment
            timeseries (pd.DataFrame): Index of each constituent

        Returns:
            pd.DataFrame: Backtested portfolio
        """
    ...
    def backtest_strategy(self) -> pd.DataFrame:
        """
        Backtest portfolio with given strategy of rebalancing
